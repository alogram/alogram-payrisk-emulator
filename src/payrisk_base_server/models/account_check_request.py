...
from payrisk_base_server.models.account import Account
...
class AccountCheckRequest(BaseModel):
    ...
    AccountCheckRequest
    """ # noqa: E501
    event_subtype: StrictStr = Field(description="The specific subtype of the account event (e.g., 'signup', 'login').", alias="eventSubtype")
    entities: Optional[EntityIds] = None
    account: Optional[Account] = None
    interaction: Optional[Interaction] = None
    __properties: ClassVar[List[str]] = ["eventSubtype", "entities", "account", "interaction"]

    @field_validator('event_subtype')
    def event_subtype_validate_enum(cls, value):
        """Validates the enum"""
        if value not in ('signup', 'login', 'password_reset', 'mfa_reset', 'setting_change',):
            raise ValueError("must be one of enum values ('signup', 'login', 'password_reset', 'mfa_reset', 'setting_change')")
        return value
    ...
    def from_dict(cls, obj: Dict) -> Self:
        ...
        _obj = cls.model_validate({
            ...
            "interaction": Interaction.from_dict(obj.get("interaction")) if obj.get("interaction") is not None else None
        })
        return _obj
