...
class DeviceInfo(BaseModel):
    ...
    fingerprint: Optional[Annotated[str, Field(min_length=35, max_length=67)]] = Field(
        default=None,
        description='Unique hash representing the device\'s fingerprint. Lowercase hex. Prefix "fp_". ',
    )
    user_agent: Optional[Annotated[str, Field(min_length=10, max_length=512)]] = Field(
        default=None,
        ...
        _obj = cls.model_validate(
            {
                ...
                "ip": (
                    IpInfo.from_dict(obj.get("ip"))
                    if obj.get("ip") is not None
                    else None
                ),
                "integrity": (
                    Integrity.from_dict(obj.get("integrity"))
