...
class PaymentMethodBase(BaseModel):
    ...
    """ # noqa: E501
    type: StrictStr = Field(description="The payment instrument type for this purchase.")
    __properties: ClassVar[List[str]] = ["type"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in ('card', 'wallet', 'realtime', 'bank_transfer', 'crypto', 'invoice',):
            raise ValueError("must be one of enum values ('card', 'wallet', 'realtime', 'bank_transfer', 'crypto', 'invoice')")
        return value
